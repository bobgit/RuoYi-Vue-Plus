...
COMMENT ON COLUMN ecom_product_template.status          IS '状态（0在售 1下架 2预售）';
COMMENT ON COLUMN ecom_product_template.audit_status    IS '审核状态（0待审核 1通过 2拒绝）';
COMMENT ON COLUMN ecom_product_template.del_flag        IS '删除标志（0存在 1删除）';
...
CREATE TABLE IF NOT EXISTS ecom_product_spu (
    ...
    owner_id   INT8,
    product_type VARCHAR(20) DEFAULT 'physical', -- 业务类型
    spu_name        VARCHAR(200)  NOT NULL,
    spu_desc        TEXT,
    packing_list    VARCHAR(1000),
    after_service   VARCHAR(1000),
...
COMMENT ON COLUMN ecom_product_spu.product_type     IS '分类：physical（实物）virtual（虚拟卡密）ticket（电子票券）food_delivery（外卖）service（到店服务，如按摩、美发）course（在线课程）travel（景区门票/旅游项目）seckill（秒杀商品）服务 / 二手';
...
COMMENT ON COLUMN ecom_product_spu.status          IS '状态（0在售 1下架 2预售）';
COMMENT ON COLUMN ecom_product_spu.audit_status    IS '审核状态（0待审核 1通过 2拒绝）';
COMMENT ON COLUMN ecom_product_spu.del_flag        IS '删除标志（0存在 1删除）';
...
CREATE TABLE IF NOT EXISTS ecom_product_sku (
    ...
    stock_type VARCHAR(20) DEFAULT 'finite'
...
COMMENT ON COLUMN ecom_product_sku.original_price IS '原价（单位：分）';
COMMENT ON COLUMN ecom_product_sku.group_price    IS '团购价（单位：分）';
COMMENT ON COLUMN ecom_product_sku.stock_type    IS 'finite: 有限库存 infinite: 无限库存（虚拟商品/票券）daily_limit: 每日限售';
COMMENT ON COLUMN ecom_product_sku.stock_quantity IS '库存数量';
...
COMMENT ON COLUMN ecom_activity_product.activity_stock      IS '活动库存';
...
COMMENT ON TABLE ecom_logistics_history IS '通用库存表';
COMMENT ON COLUMN ecom_logistics_history.history_id    IS '库存ID';
COMMENT ON COLUMN ecom_logistics_history.tenant_id     IS '租户编号';
COMMENT ON COLUMN ecom_logistics_history.sku_id      IS 'sku Id';
